...
export default function CityTraffic() {
  ...
  // Lecture de la config admin pour la fluidité (Basse / Moyenne / Élevée)
  const config = useAdminConfig();
  const fluidityQuality = config?.trafficFluidity || "medium"; 
  
  // Fréquence d'allègement de l'IA (1 = max, 2 = 1 frame sur 2, 3 = 1 frame sur 3)
  const updateInterval = fluidityQuality === "low" ? 3 : fluidityQuality === "medium" ? 2 : 1;
  const frameCount = useRef<number>(0);

  // Charger le trafic importé par l'utilisateur
  useEffect(() => {
    ...
    const specs: CarSpec[] = customList.map((c, i) => ({
      ...
      color: "#ffffff",
      accent: "#cccccc",
      duration: 25 + Math.random() * 15, // Vitesse de parcours du trajet
      delay: -(Math.random() * 40),
      pathIdx: i % ROADS.length === 1 ? 0 : i % ROADS.length, // Éviter l'index 1 exclu
      side: Math.random() > 0.5 ? 1 : -1, // Choix de la voie (droite ou gauche)
      kind: "sedan",
      imageUrl: c.imageUrl,
      category: c.category
    }));
    
    // CULLING OPTIMISATION : Réduction drastique du spawn max si la config est en mode basse qualité
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      for (let i = 0; i < customCars.length; i++) {
        ...
        // --- OPTIMISATION DU CALCUL PROCESSEUR ---
        // On alterne les calculs de trajectoires complexes à l'aide du Modulo.
        // Les calculs lourds sautent des tours sans impacter l'affichage visuel.
        const skipHeavyCalculation = i % updateInterval !== frameCount.current % updateInterval;
        if (skipHeavyCalculation) {
          continue; 
        }

        // Calcul de la progression temporelle le long du tracé quadratique SVG
        const progressTime = timeSec + car.delay;
        const rawFrac = (progressTime % car.duration) / car.duration;
        const frac = car.side === -1 ? 1 - rawFrac : rawFrac; // Gère le sens de circulation
        const currentLen = frac * totalLen;

        const p = path.getPointAtLength(currentLen);
        // Culling de rendu : Ne pas traiter si c'est en dehors des limites de la map
        ...
        // Décalage strict à droite pour appliquer la séparation des voies (LANE_HALF)
        const nx = -dy / L * LANE_HALF * car.side;
        const ny = dx / L * LANE_HALF * car.side;

        // Calcul de l'orientation vectorielle du véhicule (en degrés)
        let ang = (Math.atan2(dy, dx) * 180) / Math.PI;
        if (car.side === -1) ang += 180; // Retourne le véhicule s'il roule à contresens du tracé de base

        // Injection directe dans la matrice DOM (Infiniment plus rapide qu'un re-render d'état React)
        node.setAttribute(
          "transform",
  ...
  return (
    ...
    >
      {/* Génération masquée des lignes de référence géométriques des routes */}
